...
RcppExport SEXP cwbart(
    ...
    SEXP _ipower,          // power parameter for the tree prior
    SEXP _ibase,           // base parameter for the tree prior
    SEXP _itau,            // tau parameter, sigmaf / sqrt(ntrees)
    SEXP _inu,             // prior for variance
    SEXP _ilambda,         // scale of the prior for variance
    SEXP _isigest,         // prior of the residual variance
    SEXP _iw,              // vector of weights that multiply standard deviation, fixed at one for the regresion 
    SEXP _idart,           // bool, run DART or not
    SEXP _itheta,          // theta parameter
    SEXP _iomega,          // omega parameter
    SEXP _igrp,            // it looks never being used
    SEXP _ia,              // sparse parameter Beta(a,b) prior, DART
    SEXP _ib,              // sparse parameter Beta(a,b) prior, DART
    SEXP _irho,            // sparse parameter, DART
    SEXP _iaug,            // bool, whether data augmentation is being used, DART
    SEXP _inkeeptrain,     // number of MCMC returned for train
    SEXP _inkeeptest,      // number of MCMC returned for test
    SEXP _inkeeptestme,    // number of MCMC returned for test mean
    SEXP _inkeeptreedraws, // number of MCMC returned for tree draws
    SEXP _inprintevery,    // bool, print message of every iteration
                           //   SEXP _treesaslists,
    SEXP _Xinfo            // choose your own cutpoints
)
{
    ...
    double *_tedraw)
{
   ...
   for (size_t i = 0; i < (nd + burn); i++)
   {
      ...
      if (i == (burn / 2) && dart)
         bm.startdart();
      //draw bart, svec is a length n vector of sigma for each observation. gen is random number generator
      bm.draw(svec, gen);
      //draw sigma
      rss = 0.0;
      for (size_t k = 0; k < n; k++)
      {
         restemp = (iy[k] - bm.f(k)) / (iw[k]);
         rss += restemp * restemp;
      }
