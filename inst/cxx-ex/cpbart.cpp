...
RcppExport SEXP cpbart(
    ...
    SEXP _binaryOffset, // offset of the probit link
    SEXP _itau,
...
*/
   ...
   for (size_t i = 0; i < (nd + burn); i++)
   {
      ...
      if (i == (burn / 2) && dart)
         bm.startdart();
      //draw bart
      // here the input sigma is 1
      // it is still gaussian case, so the partial residual processes are the same
      // might need to modify the bm class for poisson classification
      bm.draw(1., gen);
      //bm.draw(rootM, gen);
